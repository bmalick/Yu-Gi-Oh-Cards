...
class YGODeck:
    ...
    def get_keys(self):
        ...
        token, db_id = content.split("\n")[:2]
        db_id = db_id.split("?")[0].split("/")[-1]
        return token, db_id
    ...
    def fetch(self):
        ...
        try:
            token, db_id = self.get_keys()
        except: 
            return "Notion api key or database link is not well entered. Use change keys button please"
        try:
            ...
            for card in cards:
                ...
                for i in range(len(card["card_images"])):
                    ...
                    cards_instances.append(card_class)
